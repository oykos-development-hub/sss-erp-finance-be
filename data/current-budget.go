...
func (t *CurrentBudget) GetBy(condition up.AndExpr) (*CurrentBudget, error) {
	...
	if err != nil {
		if goerrors.Is(err, up.ErrNoMoreRows) {
			return nil, newErrors.WrapNotFoundError(err, "upper one")
		}
...
func (t *CurrentBudget) UpdateActualWithTx(ctx context.Context, tx up.Session, currentBudgetID int, value decimal.Decimal) error {
	userID, ok := contextutil.GetUserIDFromContext(ctx)
	if !ok {
		err := newErrors.New("user ID not found in context")
		return newErrors.Wrap(err, "contextuitl get user id from context")
	...
	res, err := tx.SQL().Exec(updateQuery, value, currentBudgetID)
	if err != nil {
		return newErrors.Wrap(err, "upper tx")
	}
...
func (t *CurrentBudget) GetActualCurrentBudget(organizationUnitID int) ([]*CurrentBudget, error) {
	...
	query := `WITH sorted_data AS (
			  ...
			  WHERE unit_id = $1
   			  ORDER BY id DESC)
		SELECT budget_id, account_id, actual, balance, initial_actual, current_amount, type
		FROM sorted_data
	...
	for rows.Next() {
		...
		err = rows.Scan(&item.BudgetID, &item.AccountID, &item.Actual, &item.Balance, &item.InitialActual, &item.CurrentAmount, &item.Type)
